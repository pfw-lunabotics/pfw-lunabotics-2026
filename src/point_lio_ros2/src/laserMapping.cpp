...
bool sync_packages(MeasureGroup &meas) {
    ...
    if (lidar_buffer.empty() || imu_deque.empty()) {
        ...
        if (++sync_empty_count % 500 == 0)
            printf("[sync_packages] empty: lidar_buf=%zu imu_deq=%zu\n", lidar_buffer.size(), imu_deque.size());
        return false;
    }
    ...
    if (!lidar_pushed) {
        ...
        meas.lidar_last_time = lidar_end_time;
        lidar_pushed = true;
        printf("[sync_packages] lidar pushed: beg=%.3f end=%.3f curvature_max=%.3f pts=%zu\n",
               meas.lidar_beg_time, lidar_end_time, end_time, meas.lidar->points.size());
    }

    if (last_timestamp_imu < lidar_end_time) {
        static int sync_wait_count = 0;
        if (++sync_wait_count % 500 == 0)
            printf("[sync_packages] waiting: last_imu=%.3f lidar_end=%.3f diff=%.3f imu_deq=%zu\n",
                   last_timestamp_imu, lidar_end_time, lidar_end_time - last_timestamp_imu, imu_deque.size());
        return false;
    }
...
//------------------------------------------------------------------------------------------------------
    ...
    while (rclcpp::ok()) {
        ...
        if (sync_packages(Measures)) {
            ...
            printf("[main] after Process: feats=%zu imu_need_init=%d gravity_align=%d\n",
                   feats_undistort ? feats_undistort->size() : 0,
                   p_imu->imu_need_init_, p_imu->gravity_align_);
            ...
            if (imu_en) {
                if (!p_imu->gravity_align_) {
                    while (Measures.lidar_beg_time > get_time_sec(imu_next.header.stamp) && !imu_deque.empty()) {
                        imu_last = imu_next;
                        imu_next = *(imu_deque.front());
                        imu_deque.pop_front();
                    }
